...
func TestMenusSetbootScriptDispatchesToGoBinary(t *testing.T) {
	repoRoot := repoRootFromThisFile(t)
	script := filepath.Join(repoRoot, "scripts", "menus", "4_setboot.sh")
	...
	fakeCtl := filepath.Join(fakeBin, "shellcrash-setbootctl")
	body := "#!/bin/sh\nprintf '%s' \"$*\" > \"$SC_MARKER\"\n"
	if err := os.WriteFile(fakeCtl, []byte(body), 0o755); err != nil {
		t.Fatalf("write fake setbootctl binary: %v", err)
	}

	cmd := exec.Command("sh", "-c", ". \"$SC_SCRIPT\"; setboot")
	...
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("run setboot wrapper: %v, out=%s", err, string(out))
	}
