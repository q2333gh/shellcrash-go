...
func TestMenusTaskScriptDispatchesToGoTaskBinary(t *testing.T) {
	repoRoot := repoRootFromThisFile(t)
	script := filepath.Join(repoRoot, "scripts", "menus", "5_task.sh")
	...
	fakeCtl := filepath.Join(fakeBin, "shellcrash-taskctl")
	body := "#!/bin/sh\nprintf '%s' \"$*\" > \"$SC_MARKER\"\n"
	if err := os.WriteFile(fakeCtl, []byte(body), 0o755); err != nil {
		t.Fatalf("write fake taskctl binary: %v", err)
	}

	cmd := exec.Command("sh", "-c", ". \"$SC_SCRIPT\"; task_menu")
	...
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("run task menu wrapper: %v, out=%s", err, string(out))
	}
