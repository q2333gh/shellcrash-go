# "Rules" that terminal programs follow（本地存档）

> 原文链接：<https://jvns.ca/blog/2024/11/26/terminal-rules/>  
...
1. The job of whatever program you happen to be running (like`top` or`vim` or`cat`)
2. Your terminal emulator’s job
3. Your shell’s job
4. Your operating system’s job
...
1. eventually maybe formulate it into an explicit rule that I know
2. subconsciously figure out what the pattern is ("`Ctrl-C` will generally quit noninteractive programs, but in interactive programs it might interrupt the current operation instead of quitting the program")
3. notice some exceptions (“okay,`Ctrl-C` will exit`find` but not`less`”)
4. learn that the rule applied anywhere at all ("`Ctrl-C` will exit programs")
